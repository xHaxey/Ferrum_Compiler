...
{
public:
    std::vector<std::variant<Expression*, Node*>> matches;
};

Type ResolveType(std::variant<Expression*, Node*> match)
{
...
std::string ResolveIdentifier(std::variant<Expression*, Node*> match)
{
...
std::string ResolveOperator(std::variant<Expression*, Node*> match)
{
...
std::unique_ptr<Expression> ResolveExpr(std::variant<Expression*, Node*> match)
{
    return std::make_unique<Expression>(std::get<Expression*>(match));
}

std::unique_ptr<Node> ResolveNode(std::variant<Expression*, Node*> match)
{
    return std::make_unique<Node>(std::get<Node*>(match));
}

std::unique_ptr<ParamNode> ResolveParam(std::variant<Expression*, Node*> match)
{
    return std::make_unique<ParamNode>(static_cast<ParamNode*>(std::get<Node*>(match)));
}

std::unique_ptr<BlockNode> ResolveBlock(std::variant<Expression*, Node*> match)
{
...
{
    auto expr = ResolveExpr(result.matches.at(0));
